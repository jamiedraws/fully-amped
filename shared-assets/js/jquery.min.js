...
(function($) {
    $.scrollToElement = function($element, speed, params) {
		var topAdjustment = params && params.top ? params.top : 0;
        speed = speed || 750;
        if ($element.length > 0) {
            $("html, body").animate({
                scrollTop: $element.offset().top + topAdjustment
            }, speed);
        }
        return $element;
    };

    $.scrollTo = function(selector, top, speed) {
        speed = speed || "normal";
        return $.scrollToElement($(selector), speed, top);
    };
})(jQuery);
